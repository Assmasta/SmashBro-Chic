...
identify problems, frame by frame

TODO:
press psychological advantage:
positioning:
    edgeguarding from the ledge ----------> split between juggle, recover and edgeguard
frame:
    punish whiffs

neutral against ground options, such as grounded attack spam ----------> remain in shieldable frames more often

chase ----------> turn buffer to near zero, stay on inside


WHEN TO MAKE UNSAFE CHOICES:

for aggressive players: approach with shield (wait for them to attack?), wavedash in oos, crouch, shield grab
measure time spent: in attack recoil, shielding

for passive players: approach with shield (wait for them to attack?), wavedash in oos, bait, approach with attack
measure time spent standing, airdodging?, falling while close to ground (check for low aerials)

FIX:
HARD: falling aerial doesn't jump first if on platform
HARD: FoD platform exception thrown float and nonetype

chump checks:
EASY: jump out of juggles?
MEDIUM: do not allow op to land if they have no jumps
retreating needle
MEDIUM: tempo change
HARD: broad predictions and reads, stuffing approaches

HARD: variable shield-holding
...
class ESAgent():
    ...
    def act(self, gamestate):
        ...
        # if gamestate.player[self.opponent_port].position.y >= 0:
        #     tracker_trigger = True
        # if tracker_trigger:
        #     print('player action/frame:', gamestate.player[self.opponent_port].action,
        #           gamestate.player[self.opponent_port].action_frame,
        #           'coordinates:', round(gamestate.player[self.opponent_port].position.x, 3),
        #           round(gamestate.player[self.opponent_port].position.y, 3))
        #     print('bot action/frame:', gamestate.player[self.smashbro_port].action,
        #           gamestate.player[self.smashbro_port].action_frame,
        #           'coordinates:', round(gamestate.player[self.smashbro_port].position.x, 3),
        #           round(gamestate.player[self.smashbro_port].position.y, 3))

        self.action_list.append(gamestate.player[self.smashbro_port].action)
        if len(self.action_list) > 60:
            self.action_list.pop(0)

        last_action = self.action_list[len(self.action_list) - 2]
        ...
        # Sword dance check
        if gamestate.player[self.smashbro_port].action in [Action.SWORD_DANCE_1_AIR]:
            self.has_danced = True
        if gamestate.player[self.smashbro_port].on_ground:
            self.has_danced = False
        ...
        gamestate.custom["has_danced"] = self.has_danced
        ...
        gamestate.custom["last_action"] = last_action
