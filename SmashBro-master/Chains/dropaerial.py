...
class DROP_AERIAL_DIRECTION(Enum):
    UP = 0
    DOWN = 1
    FORWARD = 2
    BACK = 3
...
# edge drop aerial
class DropAerial(Chain):
    def __init__(self, direction=DROP_AERIAL_DIRECTION.NEUTRAL):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if smashbro_state.action in aerial:
            self.interruptible = True
        ...
        if opponent_state.position.y > smashbro_state.position.y:
            self.interruptible = True
            controller.release_all()
            return
        ...
        # falling down
        if smashbro_state.speed_y_self < 0 and smashbro_state.action in actionable_falling:
            if self.direction == DROP_AERIAL_DIRECTION.UP:
                controller.tilt_analog(Button.BUTTON_C, .5, 1)
                controller.tilt_analog(Button.BUTTON_MAIN, .5, .5)
            if self.direction == DROP_AERIAL_DIRECTION.DOWN:
                controller.tilt_analog(Button.BUTTON_C, .5, 0)
                controller.tilt_analog(Button.BUTTON_MAIN, .5, .5)
            if self.direction == DROP_AERIAL_DIRECTION.FORWARD:
                controller.tilt_analog(Button.BUTTON_C, int(smashbro_state.facing), .5)
                controller.tilt_analog(Button.BUTTON_MAIN, .5, .5)
            if self.direction == DROP_AERIAL_DIRECTION.BACK:
