...
class SDI(Chain):
    ...
    def touching_ground(smashbro_state):
        ...
        if abs(smashbro_state.position.y) < 0.25:
            return True
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if opponent_state.character == Character.SHEIK and opponent_state.action == Action.SWORD_DANCE_2_HIGH:
            if gamestate.frame % 2:
                controller.tilt_analog(Button.BUTTON_MAIN, int(opponent_state.position.x > smashbro_state.position.x), 0.5)
            else:
                controller.tilt_analog(Button.BUTTON_MAIN, 0.5, 0.5)
            return

        # SDI in for crouch cancel
        if smashbro_state.action in [Action.CROUCHING, Action.CROUCH_START, Action.CROUCH_END] and smashbro_state.on_ground:
            if gamestate.frame % 4:
                controller.tilt_analog(Button.BUTTON_MAIN, int(opponent_state.position.x > smashbro_state.position.x), 0.5)
                print('SDI in for CC')
            else:
                controller.tilt_analog(Button.BUTTON_MAIN, 0.5, 0.5)
        ...
        if self.cardinal is not None:
            ...
            if SDI.touching_ground(smashbro_state):
                ...
                if self.cardinal[1] == 0.5:
                    # raised from 2 to make it a little less insane
                    if gamestate.frame % 3:
                        controller.tilt_analog(Button.BUTTON_MAIN, 0.5, 0.5)
                    else:
        ...
        if opponent_state.character in [Character.PEACH, Character.PIKACHU, Character.SAMUS, Character.SHEIK] and opponent_state.action == Action.DOWNSMASH:
            angle = math.degrees(-math.atan2(smashbro_state.speed_x_attack, smashbro_state.speed_y_attack)) + 90
        ...
        # ASDI down weak hits to become actionable asap
        if smashbro_state.percent < 60 and 0 < smashbro_state.position.y < 5 and not smashbro_state.off_stage:
        ...
        if smashbro_state.off_stage:
            cardinal = (int(smashbro_state.position.x < 0), int(smashbro_state.position.y < 0))
            if self.logger:
        ...
        if hi_com or smashbro_state.percent > 60 and absolute_speed > 3:
            ...
            if smashbro_state.position.y < 6:
                self.cardinal = (int(smashbro_state.position.x < 0), 0)
            else:
                angle = math.degrees(-math.atan2(smashbro_state.speed_x_attack, smashbro_state.speed_y_attack)) + 90
                ...
                self.cardinal = SDI.angle_to_cardinal(angle)
                if self.logger:
                    self.logger.log("Notes", " Survival SDI angle: " + str(angle) + " " + str(smashbro_state.speed_y_attack) + " " + str(smashbro_state.speed_x_attack), concat=True)
