...
class ExactDash(Chain):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        lastdashframe = 7
        if smashbro_state.action == Action.DASHING and smashbro_state.action_frame >= lastdashframe:
        ...
        if abs(smashbro_state.position.x) > \
            melee.stages.EDGE_GROUND_POSITION[gamestate.stage] - 5.4:#(3 * SHIEK_DASH_SPEED):
                x = 0
                if smashbro_state.position.x < 0:
                    x = 1
