...
class DashDance(Chain):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        # TODO different from wavedance here
        # Dash back, since we're about to start running
        if smashbro_state.action == Action.DASHING and smashbro_state.action_frame >= 7:
        ...
        jumpcancel = (smashbro_state.action == Action.KNEE_BEND) and (smashbro_state.action_frame == 4)
        if jumpcancel or (smashbro_state.action in jumping and abs(smashbro_state.position.x) < melee.stages.EDGE_GROUND_POSITION[gamestate.stage] and -5 < smashbro_state.position.y < 5):
            ...
            # TODO choose wavedash direction
        ...
        if abs(smashbro_state.position.x) > \
            melee.stages.EDGE_GROUND_POSITION[gamestate.stage] - 5.4:#(3 * SHEIK_DASH_SPEED):
                x = 0
                if smashbro_state.position.x < 0:
                    x = 1
