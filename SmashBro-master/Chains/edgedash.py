...
class Edgedash(Chain):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if smashbro_state.action == Action.JUMPING_ARIAL_FORWARD:
            ...
            if smashbro_state.position.y + smashbro_state.ecb.bottom.y > 0.25:
                airdodge_angle = 0.2
                # Go at a more horizontal angle if we're only just barely above the stage
                if smashbro_state.position.y + smashbro_state.ecb.bottom.y < 1:
                    airdodge_angle = 0.35

                self.interruptible = False
                controller.tilt_analog(Button.BUTTON_MAIN, int(smashbro_state.position.x < 0), airdodge_angle)
