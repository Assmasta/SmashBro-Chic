...
class Defend(Tactic):
    def needsprojectiledefense(smashbro_state, opponent_state, gamestate, logger=None):
        if smashbro_state.invulnerability_left > 2:
            return False
        ...
        for projectile in gamestate.projectiles:
            ...
            if abs(smashbro_state.position.x - projectile.position.x) < 10 and abs(projectile.speed.x) < 1:
                size += 5
            ...
            for i in range(0, 1):
                ...
                smashbro_y = smashbro_state.position.y
                smashbro_x = smashbro_state.position.x + smashbro_state.speed_ground_x_self
    ...
    def needsdefense(smashbro_state, opponent_state, gamestate, framedata):
        # Is opponent attacking?
        if smashbro_state.invulnerability_left > 2:
            return False

        # to hold shield
        if smashbro_state.action in [Action.SHIELD, Action.SHIELD_START, Action.SHIELD_STUN, Action.SHIELD_REFLECT]:
            return True

        # for getup attacks
        if opponent_state.action in [Action.GROUND_ATTACK_UP, Action.GETUP_ATTACK] and opponent_state.action_frame <= 26:
            return True
        ...
        # Try to get the ledge from Sheik Shino stall first
        ...
        # Prevent shield-breaking oneself on Peach recovery
        if opponent_state.character == Character.PEACH and opponent_state.action in [Action.PARASOL_FALLING] and opponent_state.position.y < 0:
            return False
        ...
        if framedata.is_grab(opponent_state.character, opponent_state.action) and \
                smashbro_state.action in [Action.EDGE_HANGING, Action.EDGE_CATCHING]:
            return False

        # Will we be hit by this attack if we stand still?
        hitframe = framedata.in_range(opponent_state, smashbro_state, gamestate.stage)
        # Only defend on the edge if the hit is about to get us
        if smashbro_state.action in [Action.EDGE_HANGING, Action.EDGE_CATCHING] and hitframe > 2:
            return False
        if hitframe:
            return True
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if Defend.needsprojectiledefense(smashbro_state, opponent_state, gamestate):
            for projectile in projectiles:
                ...
                print("projectile powershield activated")
                return
        ...
        # Is the attack a grab? If so, maybe spot dodge right away
        ...
        onfront = (opponent_state.position.x < smashbro_state.position.x) == opponent_state.facing

        # Are we in the shield window?

        # powershield likelihood (frames)
        # tracker influence
        shield_response_min = 3
        shield_response_max = random.randint(4, 12)
        if gamestate.custom["grab_fraction"] < 50:
            shield_response_min = 2
            shield_response_max = 120

        multihit = [Action.GROUND_ATTACK_UP, Action.GETUP_ATTACK]

        # in case airborne
        if not smashbro_state.on_ground and not smashbro_state.off_stage and not smashbro_state.action == Action.NEUTRAL_B_CHARGING_AIR:
            self.pickchain(Chains.Airdodge, [.5, 0])
            # print('airdodge down', round(smashbro_state.position.x, 0), round(smashbro_state.position.y, 0))
            return

        # platform shield actions
        lowest_plat = 22
        if gamestate.stage != melee.enums.Stage.FINAL_DESTINATION and smashbro_state.position.y > lowest_plat:
            if smashbro_state.action in [Action.SHIELD, Action.SHIELD_REFLECT] and smashbro_state.hitlag_left == 0:
                if self.controller.prev.main_stick[0] == int(smashbro_state.facing): # if holding in the direction faced
                    self.chain = None
                    self.pickchain(Chains.ShieldDrop)
                    return

        # duct-tape fix for long-lasting aerials (maybe change to spotdodge?)
        if opponent_state.character in [Character.GANONDORF, Character.CPTFALCON, Character.SHEIK] and \
                opponent_state.action in [Action.FAIR, Action.DAIR] and \
                1 <= opponent_state.action_frame <= 12:
            self.pickchain(Chains.Wavedash, [1.0, False])
            return

        # powershield response formerly 2 frames
        if shield_response_min < framesuntilhit <= shield_response_max and not is_grab:
            if 2 <= framesuntilhit and opponent_state.action in [Action.GROUND_ATTACK_UP, Action.GETUP_ATTACK]:
                if smashbro_state.percent <= 90 and smashbro_state.action in [Action.STANDING, Action.RUNNING]:
                    hold = framedata.hitbox_count(opponent_state.character, opponent_state.action) > 1
                    self.chain = None
                    self.pickchain(Chains.Crouchcancel, [hold])
                    print('crouch cancel called')
                    return
                if smashbro_state.percent <= 40 and opponent_state.character in \
                        [Character.FALCO, Character.CPTFALCON, Character.FOX]:
                    hold = framedata.hitbox_count(opponent_state.character, opponent_state.action) > 1
                    self.chain = None
                    self.pickchain(Chains.Crouchcancel, [hold])
                    print('crouch cancel called')
                ...
                if framesuntilhit == 2:
                    self.chain = None
                    self.pickchain(Chains.Run, [smashbro_state.position.x < opponent_state.position.x])
                    return
            ...
            if smashbro_state.action in [Action.SHIELD_REFLECT] and smashbro_state.action_frame == -1:
                print("shield activated")
            if smashbro_state.action in [Action.SHIELD_START] and smashbro_state.action_frame == 1:
                print("shield activated")
            hold = framedata.hitbox_count(opponent_state.character, opponent_state.action) > 1
            self.pickchain(Chains.Powershield, [hold, False])

        # hold shield longer to give opponents a chance to shield pressure
        if smashbro_state.action in [Action.SHIELD, Action.SHIELD_START, Action.SHIELD_STUN, Action.SHIELD_REFLECT]:
            hold = framedata.hitbox_count(opponent_state.character, opponent_state.action) > 1
            if hold == False:
                hold_rng = random.randint(0, 3)
                # tracker adjustment
                if gamestate.custom["grab_fraction"] < 50:
                    hold_rng = random.randint(0, 2)
                if hold_rng != 0:
                    hold = True
                    print('shield strength:', round(smashbro_state.shield_strength, 2))
                if smashbro_state.shield_strength < 10:
                    hold = False
                    print('shield low, release')
            self.pickchain(Chains.Powershield, [hold, False])
            return
        else:
